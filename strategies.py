...
class SensationAggregator:
    ...
    def get_atc(self, thermal_sensation: dict, sim_output: dict):
        ...
        if self.strategy == "const":
            ...
            raise Exception("Here is no such strategy:", self.strategy)
...
def drift(thermal_sensation: dict, mode):
    ...
    :return: int, the aggregated vote which is from -3 to 3
    """
    # Calculate the average of the thermal sensation values
    average = sum(thermal_sensation.values()) / len(thermal_sensation)

    if mode == "htg":
        # For 'htg' mode, round the average up to make the value as large as possible
        avg = math.ceil(average)
        # Further increase the value by 1
        avg += 1
    else:
        # For other modes, round the average down to make the value as small as possible
        avg = math.floor(average)
        # Further decrease the value by 1
        avg -= 1

    # Clamp the result within the range of -3 to 3 and return
