...
class CoSimulation:
    ...
    def __init__(self, model_path, start_date: datetime, end_date: datetime, input_param: dict, output_param: dict,
                 ...
                 result: Result,
                 update_loss, epw_file_path, turn_off_when_empty, weather_path, hvac_control_strategy, spaces, space_zone_map, description_map):
        """
    ...
    def callback_function(self, state):
        ...
        if 20 <= minute <= 40:
            ...
            minute = 0
        if self.sim_time.day != day:
            print("Finished simulation for month ", month, ": ", day)
        self.sim_time = self.sim_time.replace(month=month, day=day, hour=hour, minute=minute)

        # print(self.sim_time)

        season = None
        if self.sim_time.month in [6, 7, 8]:
            season = "clg"
            self.sensation_aggregator.mode = "clg"
        elif self.sim_time.month in [1, 2, 12]:
            season = "htg"
            self.sensation_aggregator.mode = "htg"

        if season != "clg":
            return
        # if self.sim_time.month != 7:
        #     return

        # if the simulation time is in the working time
        if season is not None and 7 <= self.sim_time.hour <= 21:
            ...
            for variable, handle in self.handle_variables.items():
                ...
                if "temp" in variable:
                    if variable.lstrip("temp").isdigit():
                        ...
                        # if int(variable.lstrip("temp")) < 55:
                        #     self.spaces[int(variable.lstrip("temp"))].temperature = variable_value
                        #     self.spaces[]

            # Update temperature in Spaces
            for s_name, s_obj in self.spaces.items():
                # zone_number = self.space_zone_map[s_name].lstrip("Thermal Zone")
                if not "Space 1 - 1" == s_name and not "Space 2 - 4" == s_name:
                    s_obj.temperature = indoor_temp[int(self.space_zone_map[s_name].lstrip("Thermal Zone"))]
                    ...
                    if season in actuator:
                        ...
                        if self.turn_off_when_empty:
                            ...
                            if group_thermal_sensation[num_space] == 4:
                                ...
                                set_point = self.hvac_controller(indoor_temp[num_space],
                                                                 group_thermal_sensation[num_space],
                                                                 self.hvac_control_strategy)
                                if set_point > 30:
                                    set_point = 30
                                elif set_point < 17:
                                    set_point = 17
                            self.api.exchange.set_actuator_value(state, handle, set_point)
                            setpoint_record[num_space] = set_point
                            # if num_space < 55:
                            #     self.spaces[num_space].setpoint = set_point

                        else:
                            self.api.exchange.set_actuator_value(state, handle, 23)
                            setpoint_record[num_space] = 23
                else:
                    if "clg" in actuator:
                        self.api.exchange.set_actuator_value(state, handle, 40)
                        setpoint_record[int(re.search(r'\d.*', actuator).group(0))] = 40
                    elif "htg" in actuator:
                        self.api.exchange.set_actuator_value(state, handle, 0)
                        setpoint_record[int(re.search(r'\d.*', actuator).group(0))] = 0

            for s_name, s in self.spaces.items():
                s.setpoint = setpoint_record[int(self.space_zone_map[s_name].lstrip("Thermal Zone"))]
            ...
            self.evaluation_results.update_other_metrics()
